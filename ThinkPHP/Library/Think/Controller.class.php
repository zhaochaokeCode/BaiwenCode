...
abstract class Controller {
  ...
  /**
     * 加密算法
     */
    public function sigurl($data,$key='gTjUrlRWjdwkH3dp&'){
     ...
     * 设置csrf防御
     */
    public function SetCsrf($leng = 9999){
        session("csrf" , md5(rand(0,$leng)));
        $this -> assign("csrf",$_SESSION['csrf']);
    }

    /**
     * 验证csrf攻击
     */
    public function GetCsrf($backUrl,$text = '数据异常!'){
        if(isset($_SESSION['csrf']) && $_SESSION['csrf'] == $_POST['csrf']){
            unset($_SESSION['csrf']);
            return true;
        }else{
            echo "<script>alert('".$text."');location.href='".$backUrl."'</script>";
            return false;
            exit;
        }
    }
