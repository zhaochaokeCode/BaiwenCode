...
class NewsController extends Controller {
    public function index(){
        ...
        //数据
        $data = $news -> query("select title,content,id,is_top from th_news ".$where." order by is_top desc , createtime desc limit ".$limit.','.$listCount);
    ...
    public function top($limit){
        $news = M('news');
        return $news -> join("th_tname on th_news.type = th_tname.identification")-> where("th_news.is_top = 1") -> order("th_news.createtime DESC") -> limit($limit) -> select();
    }
}